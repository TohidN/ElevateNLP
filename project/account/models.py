...
class AccountSignup(models.Model):
    ...
    activation_notification_send = models.BooleanField(
        _("notification send"), default=False
    )
    ...
    def change_email(self, email):
        ...
        salt, hash = functions.generate_sha1(self.user.username)
...
class Profile(models.Model):
    ...
    avatar_option = models.CharField(
        _("Avatar Option"), max_length=1, choices=AVATAR_CHOICES, default="g"
    )
    avatar = models.ImageField(
        _('If you chose "Upload Image" as avatar option this image will be used.'),
        upload_to=get_avatar_photo_path,
        blank=True,
        null=True,
    )
    ...
    language = models.CharField(
        _("Choose Language"), max_length=3, choices=LANGUAGES, blank=True, null=True
    )
    country = models.CharField(
        _("Choose Country"), max_length=2, choices=COUNTRIES, blank=True, null=True
    )
    location = models.CharField(_("Location"), max_length=140, blank=True, null=True)

    public_mail = models.EmailField(
        blank=True,
        null=True,
        verbose_name=_("Public Mail: You can add a public email to your profile!"),
    )
    privacy = models.CharField(
        max_length=1,
        choices=PRIVACY_CHOICES,
        verbose_name=_("Choose who can see your profile and favorites."),
    )
    ...
    def get_avatar_url(self, size=256, crop="center", quality=100):
        ...
        if self.avatar_option == "n":
            ...
            if self.avatar:
                im = get_thumbnail(
                    self.avatar, "{0}x{0}".format(size), crop=crop, quality=quality
                )
                return im.url
